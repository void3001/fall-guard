...
// --- Wi-Fi Credentials ---
const char* WIFI_SSID = "Adorable Filter";
const char* WIFI_PASS = "hitler123";
...
char dashboard_ip[40] = "10.15.65.43";
...
int getCalibratedSound() {
  ...
  if (rawAmplitude < 50) rawAmplitude = 0;
...
// TELEMETRY SENDER
// ==========================================
void sendTelemetryData(float temp, int gasLevel, float accX, float accY, float accZ, int volume, float pres, bool isLoggingOut) {
  if(WiFi.status() == WL_CONNECTED) {
    HTTPClient http;
    String url = "http://" + String(dashboard_ip) + ":3000/api/data/ingest";
    http.begin(url);
    ...
    doc["is_logging_out"] = isLoggingOut;
    ...
    doc["bpm"] = random(70, 95); // Simulated live Heart Rate (70-94 BPM)
    doc["battery_pct"] = random(80, 100); // Or read from real battery pin
    doc["wifi_rssi"] = WiFi.RSSI();
    
    // -- Add missing fields required by the Dashboard SQLite DB --
    doc["spo2"] = random(96, 100); // Simulated SpO2 (96-99%)
    doc["humidity"] = 45.0;
    doc["smoke_detected"] = 0;
    doc["gyro_x"] = 0.0;
    doc["gyro_y"] = 0.0;
    doc["gyro_z"] = 0.0;
    doc["altitude_m"] = 0.0;
...
void setup() {
  ...
  Wire.begin(21, 22); // Explicitly define SDA=21, SCL=22 for ESP32
  if(!display.begin(SSD1306_SWITCHCAPVCC, SCREEN_ADDRESS)) {
    Serial.println(F("OLED failed"));
    // Don't freeze forever, jump out so serial works
  } else {
    display.clearDisplay();
    display.setTextColor(SSD1306_WHITE);
    display.setTextSize(1);
    display.setCursor(5, 20);
    display.print(F("Starting up..."));
    display.display();
  }
  ...
  // 4. NETWORK CONNECTION (Do this LAST)
  display.clearDisplay();
  display.setCursor(5,20);
  ...
  int attempts = 0;
  // Increase timeout to ~20 seconds (40 attempts)
  ...
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("\nFailed to connect. Proceeding offline!");
  } else {
    Serial.println("\nWiFi connected!");
...
void loop() {
  ...
  if (!systemUnlocked) {
    if (mfrc522.PICC_IsNewCardPresent() && mfrc522.PICC_ReadCardSerial()) {
      ...
      Serial.println("Unlocked with RFID: " + currentRfid);
  ...
  // -- Allow toggling lock by tapping RFID again --
  if (mfrc522.PICC_IsNewCardPresent() && mfrc522.PICC_ReadCardSerial()) {
    // Send one final packet to tell the dashboard they are logging out
    sendTelemetryData(0, 0, 0, 0, 0, 0, 0, true); 

    systemUnlocked = false;
    currentRfid = "";
    
    // Logout Beep
    digitalWrite(BUZZER_PIN, HIGH); digitalWrite(BUZZER2_PIN, HIGH);
    delay(600); 
    digitalWrite(BUZZER_PIN, LOW); digitalWrite(BUZZER2_PIN, LOW);
    
    display.clearDisplay();
    display.setTextSize(2); display.setCursor(20, 25); display.print(F("LOCKED"));
    display.display();
  ...
  // float temp = bmp.readTemperature(); // Hardware sensor commented out
  float temp = 36.5 + (random(-10, 15) / 10.0); // Simulated body temp between 35.5C and 38.0C
  ...
  // Disabled noise trigger because the buzzer itself triggers the microphone in a loop!
  ...
  // Check if we need to send telemetry (either due to timer, or emergency)
  if (millis() - lastSendTime > sendInterval || isEmergency) {
     sendTelemetryData(temp, gasLevel, accX, accY, accZ, volume, pres, false);
     lastSendTime = millis();
  }
